...
def calculate_IDG(inp, grads, position=None, bert=False):
    ...
    if position is None:
        ...
        if "cuda" in str(device):
            div_score = div_score.cpu()
            val_score = val_score.cpu()
            div_features = div_features.cpu()
        ...
        if position == 'all':
            mask = torch.ones(inp.shape[0], device=device)
            if bert:
                mask[0] = 0
                mask[-1] = 0
            else:
            ...
            mask = torch.zeros(inp.shape[0], device=device)
            mask[position] = 1
        #print(mask)
        masked_inp = inp * mask.unsqueeze(1)
        norm_inp = masked_inp / torch.norm(masked_inp)
        intm_grads = torch.sum(grads * norm_inp.unsqueeze(0), dim=2)
        return torch.sum(torch.mean(intm_grads, dim=0)).cpu().detach().numpy()
...
def execute_single(string, model, tokenizer, target, path, guid="", bert=False):
    '''
    ...
    baseline_ids = torch.zeros(input_ids.shape, dtype=torch.int64, device=device)
    ...
    dist = dist.logits # ! @srmarcballestero: modification
    ...
    grads_neg, _, _, input_forward, baseline_forward = lig.attribute(inputs=input_ids,
                                                                    baselines=baseline_ids,
                                                                    target=inferred_class, # ! @srmarcballestero: modification
    ...
    visualize_tree(coalitions, value_func, dividend_dir, p_tree, path, 'a', guid, view=True)

    return coalitions, value_func, dividend_dir, p_tree


def execute_IDG(trees, model, tokenizer, target, path, guid="", bert=False):
    # n_steps = 250
    # all_tokens = []
    # sentences = []

    if len(trees) == 1:
        return execute_single(trees[0], model, tokenizer, target, path, guid, bert=bert)

    # ! @srmarcballestero: modification
    else:
        raise NotImplementedError("@srmarcballestero: This behaviour is not intended for now")
