...
def visualize_tree(coalitions, val_score, div_dir, tree, path, chr_, guid, view=False):
    '''
    ...
    #print(coalitions)
    #print(val_score)
    d = Graph(filename=Path(path) / f'{guid}_idg_expl_image.dot',
    ...
    for u, v in tree: # rest of the edges in the tree
        if u is not None:
            ...
            if val_score[v] > max_score:
                ...
                root = v_n
            s_u = f"{val_score[u]:.8f}"
            s_v = f"{val_score[v]:.8f}"
            d_u = div_dir[u]
            d_v = div_dir[v]
...
#     if name == 'xlnet':
#         raise NotImplementedError("XLNet is not supported in this version.")
